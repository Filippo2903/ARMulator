...
ws.onmessage = function (event) {
    ...
    for (var idx in obj_list) {
        ...
        switch (obj[0]) {
            ...
            case "edit_mode":
                disableSim();
                $("#assemble").text("Start").removeClass("assemble_edit");
                refreshBreakpoints();
                break;
            ...
            case "highlightread":
            case "highlightwrite":
                var type = obj[0].slice(9); // "read" o "write"
                for (var i = 0; i < obj[1].length; i++) {
                    var element = obj[1][i];
                    try {
                        if (!isNaN(element)) {
                            // Highlight per memoria
                            if (type == "read") {
                            ...
                            // Highlight per registri
                            $(document.getElementById(element)).addClass(
                                "highlight" + type
                            );
                        }
                    } catch (e) {}
